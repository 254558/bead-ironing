...
/**
 * 图片导入：离屏画布缩采样 → 加权 RGB 最近色量化 → 居中写入网格。
 * 尺寸超出当前画布时自动扩容。
 */
  ...
  img.onload = () => {
    ...
    for (let r = 0; r < ph; r++) {
      for (let c = 0; c < pw; c++) {
        ...
        store.grid[offR + r][offC + c].color = COLORS[best]
        store.grid[offR + r][offC + c].melt = 0
      }
    }

    switchMode('design')
    showStatus('IMAGE IMPORTED! READY TO IRON')
    URL.revokeObjectURL(url)
  }

  img.onerror = () => {
    showStatus('IMAGE LOAD FAILED!')
    URL.revokeObjectURL(url)
  }
