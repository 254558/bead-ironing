 ...
 * scale 由内容尺寸动态计算（每颗珠子 3~8px，clamp 3~8）。
 * 缩略图保持适中的低分辨率，磁贴显示时用最近邻放大（image-rendering:
 * pixelated），呈现与主画布一致的像素风；珠子圆形轮廓来自抗锯齿绘制。
...
export function renderThumb(
  ...
  const scale = Math.max(3, Math.min(8, Math.round(maxSize / Math.max(cw, ch))))
